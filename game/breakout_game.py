...
class BreakoutGame():
    ...
    ---------
    comm : int
        Input generated by the agent, can be 0 or 1 (move left or right) or -1 (human input)
    """
    ...
    """Check the inputs given by a human and move the bat accordingly.
    Also checks for ESC/close button events in order to quit the game.
    We know a human is playing when the command from agent = -1.
    ...
    """ Check conditions for losing a life or reaching 'game over'.
    If ball is below screen height, lose 1 life, put a new ball into the game.
    If there is a human playing, restart the game when lives == 0.
    If there is an agent playing, call notify after when lives == 0.
